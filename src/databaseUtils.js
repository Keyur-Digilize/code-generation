...
const createRecordInDynamicTable = async (tableName, product, batch, element, code, countryCode) => {
    const uniqueId = `${element.generation_id}${element.packaging_hierarchy.replace('level', "")}${code.code}`;
...
const createRecordInCodeSummary = async (data) => {
    ...
    await prisma.codeGenerationSummary.create({
        data: {
            ...
            last_generated: String(data.no_of_codes)
        }
    });
...
const createRecordsInSsccCodes = async (data) => {
    ...
    const valuesPlaceholder = data
        .map(
            (_, index) =>
                `($${index * 7 + 1}, $${index * 7 + 2}, $${index * 7 + 3}::uuid, $${index * 7 + 4}::uuid, $${index * 7 + 5}::uuid, $${index * 7 + 6}::uuid, $${index * 7 + 7}::uuid)`
        )
        .join(", ");
